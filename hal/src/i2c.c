...
void i2c_init(i2c_type *i2cx){
	...
	* I2C
	*/
	i2cx->pI2c->CR1 |= I2C_CR1_NOSTRETCH;	// Clock stretching disabled
	...
	i2cx->pI2c->CCR	|= I2C_CLOCK / (i2cx->clockSpeed * 25);
	i2cx->pI2c->TRISE |= 0; //(I2C_CLOCK / 300000U) + 1U;
	...
	*/
	...
	i2cx->pDmaStreamTx->CR |= DMA_SxCR_PL;								// Priority level High
	...
	i2cx->pDmaStreamRx->CR |= DMA_SxCR_PL;								// Priority level High
