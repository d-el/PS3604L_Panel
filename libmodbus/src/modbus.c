    ...
    do {
        ...
        if (rc == -1) {
            ...
            if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_LINK) {
                int saved_errno = libmodbuserrno;

                if ((libmodbuserrno == EBADF || libmodbuserrno == ECONNRESET || libmodbuserrno == EPIPE)) {
                    modbus_close(ctx);
...
/* Waits a response from a modbus server or a request from a modbus client.
   ...
   and libmodbuserrno is set to one of the values defined below:
   - ECONNRESET
   - EMBBADDATA
   - EMBUNKEXC
   - ETIMEDOUT
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    if (ctx->debug) {
        if (msg_type == MSG_INDICATION) {
            modbus_printf("Waiting for an indication...\r\n");
        } else {
    ...
    if (rc != length_to_read) {
        _error_print(ctx, "error receive data size");
        return -1;
    }

	/* Display the hex code of each character received */
	if (ctx->debug) {
		char string[4 + rc * 3 + 3];
		size_t len = 0;
		len += snprintf(string, sizeof(string) - len, "Rx: ");
		for (int i = 0; i < rc; i++){
			len += snprintf(&string[len], sizeof(string) - len, "%02X ", msg[i]);
		}
		len += snprintf(&string[len], sizeof(string) - len, "\r\n");
...
/* Receives the confirmation.
   ...
   values (bits or words). Otherwise, its shall return -1 and libmodbuserrno is set.
    ...
    if (function >= 0x80) {
        if (rsp_length == (offset + 2 + (int)ctx->backend->checksum_length) &&
            ...
            if (exception_code < MODBUS_EXCEPTION_MAX) {
                libmodbuserrno = MODBUS_ENOBASE + exception_code;
            } else {
                libmodbuserrno = EMBBADEXC;
            }
            _error_print(ctx, NULL);
            return -1;
        } else {
...
/* Allocates 4 arrays to store bits, input bits, registers and inputs
   ...
   structure if successful. Otherwise it shall return NULL and set libmodbuserrno to
   ENOMEM. */
